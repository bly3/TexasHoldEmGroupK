...
class GameState:
    ...
    def _resolve_showdown(self) -> None:
        import poker_cfr.env.hand_eval as hev

        active_set = set(self.active_players)
        if not active_set:
            self.terminal = True
            return

        # Total committed per player (covers all streets, including folded players)
        contributions = [
            max(0, self.initial_stacks[i] - self.stacks[i])
            for i in range(self.num_players)
        ]

        # Build side pots from smallest contribution upward.
        contrib_remaining = contributions[:]
        pots = []
        while True:
            involved = [i for i, c in enumerate(contrib_remaining) if c > 0]
            if not involved:
                break
            level = min(contrib_remaining[i] for i in involved)
            pot_amount = level * len(involved)
            eligible = [i for i in involved if i in active_set]
            pots.append((pot_amount, eligible))
            for i in involved:
                contrib_remaining[i] -= level

        # Pre-compute hand strengths for all active players once.
        hand_strength = {
            p: hev._best_hand_score(self.hole_cards[p] + self.board)  # type: ignore[attr-defined]
            for p in active_set
        }

        awarded_winners = set()
        for pot_amount, eligibles in pots:
            if pot_amount <= 0 or not eligibles:
                continue

            # Pick winners within this eligible set by best hand strength.
            best_score = None
            winners = []
            for p in eligibles:
                score = hand_strength[p]
            ...
            # Split this pot among winners (integer chips; distribute remainder fairly).
            split = pot_amount // len(winners)
            remainder = pot_amount % len(winners)
            for idx, w in enumerate(winners):
                self.stacks[w] += split + (1 if idx < remainder else 0)
            awarded_winners.update(winners)

        self.winners = sorted(awarded_winners)
        self.pot = 0
        self.terminal = True
    ...
    def _first_to_act_postflop(self) -> int:
        ...
        if self.num_players == 2:
            return (self.button + 1) % self.num_players  # Big blind acts first postflop in HU
...
def initial_state(
    ...
    if num_players > 2:
        ...
        first_act = sb  # Heads-up: small blind/button acts first
