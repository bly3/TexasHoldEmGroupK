...
class InfrastructureConfig:
    ...
    start_key: Optional[str] = field(default_factory=lambda: os.getenv("START_KEY") or "supersecret")
    min_raise: int = 10  # heuristic floor to avoid "raise below minimum" errors (server BB defaults to 10)
...
class InfrastructureBotClient:
    ...
    def _build_observation(
        ...
        observation = {
            ...
            "street": str(state.get("phase") or table.get("phase") or "preflop").lower(),
    ...
    def _map_action(
        ...
        min_raise = max(1, int(self.config.min_raise))
        # Best-effort sizing: use pot as a proxy; fall back to min_raise to clear server's raise floor.
        pot_proxy = pot if pot > 0 else min_raise
        ...
        if normalized in {"bet_pot", "bet_small", "bet_allin"}:
            ...
            if normalized == "bet_pot":
                raise_size = max(min_raise, min(stack, pot_proxy))
            else:  # bet_small
                half_pot = max(1, pot_proxy // 2)
                raise_size = max(min_raise, min(stack, half_pot))
            if raise_size <= 0:
                return "CALL", None
